...
```{r}
dvpt_indicators = inner_join(list_country,dvpt_indicators,join_by(`Country Name`))
```

### Changing strings into numbers

We saw just after loading the data that type of the columns containing the data of 2020, 2021 and 2022 was character (except for the column 2022 of the governance data frame). We make sure that we read those data as numerical value and not as strings :
```{r}
